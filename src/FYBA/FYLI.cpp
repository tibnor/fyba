...
short LI_TestIdx(char *szSosFil)
{
   ...
   // Hvis det er valgt spesiell indekskatalog skal indeksen alltid bygges opp pÂ nytt
...
CD                         LI_OPPTATT: Filen er Âpen i et annet program
CD                         LI_FEIL_INDEKSVERSJON: Feil indeksversjon
CD                         LI_FEIL_STORRELSE: Feil st¯rrelse pÂ SOSI-filen
CD                         LI_FEIL_OPPDATTID: Feil oppdateringstid pÂ SOSI-filen
CD
CD Bruk:
...
short LI_OpenRead(LC_FILADM *pFil)
{
   ...
   /* -------------- Admin ------------- */
   // Her sjekkes det ogsÂ mot feil versjon, lesefeil
   // og om filen er Âpen i annet program.
...
/*
AR-881123
CH LC_EraseBt                                   Slett omrÂde i brukttabellen
CD ==========================================================================
CD FormÂl:
...
/*
AR-910827
CH LI_EraseBt                                   Slett omrÂde i brukttabellen
CD ==========================================================================
CD FormÂl:
CD Blanker en eller flere kolonner i brukttabellen i aktuell base.
CD
...
CD Samtidig er det mulig Â utf¯re logiske operasjoner mellom de to kolonnene.
CD
CD Parametre:
CD Type   Navn        I/U   Forklaring
CD --------------------------------------------------------------------------
...
static short LI_ReadAdm(LC_FILADM *pFil)
{
   ...
   { 
      /*
      // Sjekk om filen er Âpen i et annet program
      if (pFil->sIdxOpen == UT_TRUE)
      {
         // Sjekk om dette programmet er aktivt nÂ, eller om det har krasjet
         // (ulPid er prosessID for programmet som Âpnet filen)
         if (pFil->ulPid != 0)
         {
            HANDLE hProgram = OpenProcess( PROCESS_QUERY_INFORMATION, FALSE, pFil->ulPid );
            if (hProgram != NULL)
            {
               // Programmet finnes / er aktivt nÂ
      ...
      */
      ...
      // Ikke Âpen i annet program som er aktivt nÂ
      if (sStatus == UT_OK)
      {
...
CD ==========================================================================
CD FormÂl:
CD Beregn n¯dvendig plass for Â skrive gitt gruppe til RB.
CD
CD Parametre:
CD Type            Navn   I/U   Forklaring
CD --------------------------------------------------------------------------
...
CD long            lNko    i    Ant koord
CD unsigned long   ulPiLen i    Ant tegn som skal leses til buffer
CD long            lLen    r    Ant byte for Â lagre gruppen
CD
CD Bruk:
...
static void LI_OpenRb(LC_FILADM *pFil,UT_INT64 n64FilPos,short sModus)
{
   if (pFil->pBase->pCurRb != pFil) {
      // ----- Feil fil er Âpen, stenger denne og Âpner rett fil
      ...
      // Byggr opp fullt filnavn til Rb-filen og Âpner filen
...
CD Har innvirkning for virkemÂten til:
...
CD gruppe er satt, blir ogsÂ eventuelle refererte grupper merket.
CD
CD Parametre:
CD Type    Navn      I/U   Forklaring
CD --------------------------------------------------------------------------
...
CD                         (Lovlig BT_MIN_USER - BT_MAX_USER)
CD short   sBryter    i    Bryter   1=pÂ, 0=av
...
SK_EntPnt_FYBA long LC_MerkGr(short sKolonne,short sBryter)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {
      ...
      if (sKolonne >= BT_MIN_BT  &&  sKolonne <= BT_MAX_BT) {
         ...
         /* Er det referanser pÂ gruppen og rekursiv merking er aktivisert? */
...
CD                        (Lovlig 0 til LC_MAX_ANT_PRIOR-1)
CD short    sAvPaa   r    Av eller PÂ  (UT_TRUE = PÂ, UT_FALSE = Av)
CD
CD Bruk:
...
CD ==========================================================================
CD FormÂl:
CD Blank ut alle prioritets-bit pÂ alle gruppene i denne filen.
CD
CD Parametre:
CD Navn        Type    I/U   Forklaring
CD --------------------------------------------------------------------------
