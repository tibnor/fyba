...
CD
CD Rutiner for Â handtere hodet pÂ SOSI-filer direkte pÂ filen.
CD  ==============================================================
*/
...
CD =============================================================================
CD FormÂl:
CD Henter omrÂdeangivelsen fra filhodet.
CD
CD Parametre:
CD Type     Navn        I/U   Forklaring
CD -----------------------------------------------------------------------------
...
short ho_GetOmr(FILE *fil,double *nv_a,double *nv_n,double *oh_a,double *oh_n)
{
   ...
   /* Sjekker at omrÂdet har utstrekning */
...
CD Hvis omrÂde ikke har noen utstrekning justeres
...
CD double   nv_a          i    OmrÂde:  Nedre venstre hj¯rne
CD double   nv_n          i
CD double   oh_a          i             Øvre h¯yre hj¯rne
CD double   oh_n          i
CD short    sStatus       r    Status: UT_TRUE  = Funnet
CD                                     UT_FALSE = Ikke funnet
CD
            ...
            double nv_a,double nv_n,double oh_a,double oh_n)
{
   ...
   // Hvis n¯dvendig justeres omrÂde
...
short ho_TestSOSI(FILE *pFil,UT_INT64 *sluttpos)
{
   ...
   *sluttpos = 0;
   ...
   if (ho_FinnHode(pFil, &startpos) == UT_TRUE) {
      ...
      // Skann siste del av filen for Â finne .SLUTT
      ferdig = 0;
   ...
   /* Sjekk at hodet har transpar og fornuftig omrÂde */   
   if (ist == UT_TRUE) {
...
/*
HT:1998-05-19
CH HO_SjekkTegnsett                              Sjekker tegnsett pÂ SOSI-filen
CD =============================================================================
CD FormÂl:
...
TS_ISO8859 198 216 197 230 248 229 */
   ...
   while (!ferdig  &&  ! lesefeil ){
      ...
      for (ch=tx; *ch; ch++) {
         ...
         /* Hvis vi har fÂtt sjekket tegnsettet, og det
         ikke er funnet tidligere tas det vare pÂ nÂ */
...
short ho_FinnHode(FILE *pFil, UT_INT64 *lHodepos)
{
   ...
   *lHodepos = 0L;
   ...
   // ----- Har nÂ funnet en linje som inneholder logisk informasjon

   // Hopp over blanke pÂ starten av linjen
   cp = &tx[0];
   while (UT_IsSpace(*cp)) {
      ++cp;
      ++(*lHodepos);
