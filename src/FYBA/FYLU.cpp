...
CD ==========================================================================
CD FormÂl:
CD Frigir alle elementene (utvalgslinjene) pÂ et nivÂ i kjeden av
CD utvalgselementer for et utvalg.
CD
CD Parameters:
CD Type                  Navn      I/O  Forklaring
CD -------------------------------------------------------------------------
CD LC_UTVALG_ELEMENT *  pForsteUE  i   Start utvalgs-kjede pÂ dette nivÂ.
CD
CD Bruk:
...
SK_EntPnt_FYBA void LU_FrigiUE (LC_UTVALG_ELEMENT * pUE)
{
   ...
   /* GÂr gjennom alle elementene pÂ dette nivÂ */
   while (pUE != NULL) {
      /* Frigi elementer pÂ underliggende nivÂ under dette objektet */
      if (pUE->pForsteUE != NULL)  LU_FrigiUE(pUE->pForsteUE);

      /* Husk hva som er neste element pÂ dette nivÂ */ 
...
CD ------------------------------------------------------------------------
CD LC_UT_ADM *UtAdm  i    Peker til administrasjonsblokk for utvalg.
CD char      *qulin  i    Linje med query-tekst. (Uten prikker pÂ f¯rste nivÂ).
...
SK_EntPnt_FYBA short LC_PutQueryLine(LC_UT_ADM *pUtAdm,const char *qulin,short sType)
{
   ...
   if (pUtAdm != NULL)
   {
      ...
      // Ta vare pÂ opplysning om HØYDE er brukt
      if (sStatus != UT_FALSE) {
...
CD ==========================================================================
CD FormÂl:
CD Legger inn et regelnavn pÂ siste linje i utvalgstabellen.
...
SK_EntPnt_FYBA void LC_PutLag(LC_UT_ADM *pUtAdm,LC_UTVALG *pU,const char *navn)
{
   ...
   if (pUtAdm != NULL)
   {
	   /* Det er allerede lagt inn lag pÂ dette utvalget ? */
      if (pU->pLag != NULL) { 
         ...
         if (pLag == NULL) { /* Nytt lag, mÂ opprettes og settes inn i kjeden. */
...
CD =============================================================================
CD FormÂl:
CD Leser og tolker gruppe og punktutvalg pÂ kommandofilen og legger i tabell.
CD Forutsetter at filen er Âpnet pÂ forhÂnd.
CD
CD Parametre:
CD Type       Navn    I/U   Forklaring
CD -----------------------------------------------------------------------------
CD LC_UT_ADM *pUtAdm   i    Peker til administrasjonsblokk for utvalg.
CD FILE      *pKomFil  i    Peker til "handle" for Âpnet kommandofil.
...
SK_EntPnt_FYBA short LC_LesUtvalg(LC_UT_ADM *pUtAdm,const char *pszKomFil)
{
   ...
   if (pUtAdm != NULL)
   {
      ...
      /* Starter f¯rst pÂ filen og leser  */
...
CD short        sNiv      i   NivÂ (antall prikker forran navnet)
...
SK_EntPnt_FYBA short LU_AppUE (LC_UTVALG *pU,short sNiv,const char *pszTx)
{
   ...
   if (sAktNiv < sNiv  ||  sNiv > sAktNiv) {
      ...
      return  UT_FALSE;       /* ==> Retur nÂr ulovlig sprang i nivÂ */
   }
   ...
   if ( ! LU_TolkUtvalgslinje(pNyUE,pszTx)) {
      ...
      return  UT_FALSE;       /* ==> Retur nÂr ulovlig utvalgslinje */
   }
...
static void LU_SjekkDatatype(char *pszVerdi,char szMetode,short *psType)
{
   ...
   if (*psType == LC_U_TALL)
   {
      // Hvis tallet har for mange siffer til long mÂ det hÂndteres som tekst
...
CD =============================================================================
CD FormÂl:
CD Sjekker PINFO-delen av aktuell gruppe for tilslag pÂ PUNKT-UTVALG.
CD
CD Parametre:
CD Type       Navn       I/U   Forklaring
CD -----------------------------------------------------------------------------
...
SK_EntPnt_FYBA short LC_PiTestUtvalg(LC_UT_ADM * pUtAdm,LC_UTVALG * pU,long lPnr)
{
   ...
   if (pUtAdm != NULL)
   {
      ...
      /* S¯k */
      while (pUE != NULL) {
                                          /* Linjen mÂ testes i disse tilfeller */
...
static short LU_PiTestDelutvalg(LC_UT_ADM * pUtAdm,LC_UTVALG_ELEMENT * pUE,long lPnr)
{
   ...
   while (pUE != NULL) {
      if (sForste) {
         ...
         /*
          * Elementet mÂ testes i disse tilfeller:
          *  - Har tilslag, og metode er ..OG.
          *  - Har ikke tilslag, og metode er ..ELLER.
          * Elementet kan vÊre toppen av et nytt delutvalg, kaller derfor
          * LU_PiTestDelutvalg for Â teste dette elementet.
          */
         if (( sTilslag  &&  pUE->kommando == LC_U_OG) ||
            (!sTilslag  &&  pUE->kommando == LC_U_ELLER)) {
...
static short LU_PiTestLinje(LC_UTVALG_ELEMENT * pUE,long lPnr)
{
   ...
   if (metode == LC_U_IKKE){
      ...
      tilslag = 0;
      pUE->metode = LC_U_LIK;                         /* Sjekker f¯rst pÂ likhet */
         /* Hent parameter */
      while ((para = LC_GetPiVerdi(pUE->sosi,lPnr,&sSett)) != NULL) {
         if (LU_ParaTest(pUE,para,akt_para,U_PARA_LEN)) {              /* Tilslag? */
            tilslag = 1;
            break;          /* Vet nÂ at det ikke blir tilslag, hopper ut */
         }
         sSett++;
      }
      pUE->metode = LC_U_IKKELIK;
      if (! tilslag)              /* Tilslag nÂr "=-testen" ikke ga tilslag */
         return UT_TRUE;
...
CD som har tilslag pÂ utvalgsblokken pUtAdmFlate og har har referanser til grupper
CD som har tilslag pÂ utvalgsblokken pUtAdmOmkrets.
...
CD short           sAlle     i    Flagg for hvorvidt utvalg for omkrets mÂ slÂ til pÂ 
CD                                alle gruppene i omkretsen. TRUE/FALSE
CD short           antall    r    Antall tilslag pÂ utvalget.
CD 
CD Bruk:
		   ...
		   if (FlateBgr.lNr != 0L) {
			   ...
			   if (gnavn == L_FLATE) { // Er det en flate ?
				   if (ngi > 0){                    // Med koordinat ?
					   if (LC_GiQuery(pUtAdmFlate)){   // Tilslag pÂ flata.
...
SK_EntPnt_FYBA short LU_GiTestUtvalg(LC_UT_ADM * pUtAdm,LC_UTVALG *  pU)
{
   ...
   // Innf¯rt at ogsÂ utvalgsregler kan slÂs av for tegning. JAØ-20020927
...
static short LU_GiTestDelutvalg(LC_UT_ADM * pUtAdm,LC_UTVALG_ELEMENT * pUE)
{
   ...
   while (pUE != NULL) {
      if (sForste) {
          ...
          *  - Har ikke tilslag, og metode er ..ELLER.
          * Elementet kan vÊre toppen av et nytt delutvalg, kaller derfor
          * LU_GiTestDelutvalg for Â teste dette elementet.
          */
         if (( sTilslag  &&  pUE->kommando == LC_U_OG) ||
            (!sTilslag  &&  pUE->kommando == LC_U_ELLER)) {
            sTilslag = LU_GiTestDelutvalg(pUtAdm,pUE);
...
static short LU_ParaTest(LC_UTVALG_ELEMENT * pUE,char *para,char *pszAktPara,short sMaxLen)
{
    ...
    * nÂr dette er gitt som del av utvalgslinjen
   ...
   { 
      ...
      {
         // Tallet har for mange siffer, mÂ hÂndtere sammenligningen som tekst
         type = LC_U_ALFA;
      }
   }
...
CD short      max_len  i    Max lengde pÂ akt_para
CD
CD Bruk:
...
CD =============================================================================
CD FormÂl:
CD Bruker query-tekst for Â finne linje der parameter skal hentes.
CD
CD Parametre:
CD Type     Navn   I/U   Forklaring
CD ---------------------------------------------------------------------------
CD char    *qulin   i    Linje med query-tekst.
CD unsigned short    iniv    i    NivÂ: Det er definert konstanter som henges
CD                             sammen med "|".
CD                             LC_GINFO = s¯k i GINFO pÂ aktuell gruppe
CD                             LC_HODE = s¯k i filhodet
CD                             Hvis begge er brukt s¯kes det f¯rst i GINFO.
CD unsigned short   *univ    u    NivÂ: LC_GINFO = parameter er fra GINFO
...
CD char    *pszTx     i   Lest linje
CD short   *psNiv     u   NivÂ (antall prikker forran navnet)
...
CD ==========================================================================
CD FormÂl:
CD Sjekker GINFO og PINFO for Â finne hvilke prioriteter som "ber¯rer" aktuell
   ...
   if (pUtAdm != NULL)
   {
      ...
      if (Sys.GrId.lNr != INGEN_GRUPPE) {
         ...
         while (pU != NULL) {
            ...
            /* Hvis vi har tilslag pÂ denne prioriteten trenger vi ikke Â sjekke dette utvalget */
...
CD (Avviket er mindre enn 1/10 enhet bÂde nord og ¯st)
CD
CD Parametre:
CD Type     Navn    I/U Forklaring
CD ---------------------------------------------------------------------------
