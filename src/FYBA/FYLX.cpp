                         ...
                         unsigned char *ref_status,long max_ref)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {                  /* Aktuell gruppe OK */
      ...
      /* Les inn resten av pÂbegynnt refferert flate */
...
CD Dette kallet kan brukes til Â finne hvor stor array som mÂ allokeres f¯r
...
{
   #define MAX_LEN  66   /* Ginfolinjen skrives ut nÂr den er lengre en 66 tegn */
   short gilin;
   long j;
                                      ...
                                      short *ngi, long *nko, LC_BGR *pNyBgr)
{
   ...
   {
      // Beregner et nytt "fiktivt" punkt pÂ motsatt side av sirkelen   
      da = a1 - as;
      dn = n1 - ns;
      a2 = as - da;
      n2 = ns - dn;
   ...
   // Beregner et nytt punkt midt pÂ buen fra P1 til P2
   fiNy = fi1 + (dfi2 / 2.0);
   ...
   // Beregner et nytt punkt pÂ motsatt side av sirkelen av midtpunktet i den andre halvparten av sirkelen  
   da = aNy - as;
   dn = nNy - ns;
   aNy = as - da;
   nNy = ns - dn;
      ...
      if (Sys.pGrInfo->gnavn == L_BUE) {                 /* .BUE */
         ...
         if (LC_GetBuep(buff_retning,&a,&n,&a2,&n2,&a3,&n3)) {
            if (GM_KonvBuep(a,n,a2,n2,a3,n3,&(*as),&(*ns),&(*radius),&(*fi),&(*dfi))){
               ...
               *sfeil = 2;
               UT_FPRINTF(stderr,"Ulovlig bue: \"%s : %s\" - punktene ligger pÂ rett linje\n",Sys.GrId.pFil->pszNavn,LX_GetGi(1));
               Sys.GrId.pFil->usDataFeil |= LC_DATAFEIL_BUE;
            }
         }
                ...
                double *radius,short *storbue)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {                  /* Aktuell gruppe OK */
      if (Sys.pGrInfo->gnavn == L_BUE  &&  Sys.pGrInfo->nko > 1) {
         ...
         UT_FPRINTF(stderr,"Ulovlig bue: \"%s : %s\" - for fÂ koordinater\n",Sys.GrId.pFil->pszNavn,LX_GetGi(1));
         Sys.GrId.pFil->usDataFeil |= LC_DATAFEIL_BUE;
      }                              
   }
...
/*
OJ-891126
CH LC_GetCurEnhet                                  Hent enhet pÂ angitt nivÂ
CD ==========================================================================
CD FormÂl:
...
CD short      *nivaa    iu   angir nivÂ for henting, returnerer aktuelt nivÂ
CD                             1 = filhode
CD                             2 = GINFO
...
/*
OJ-900103
CH LC_GetCurKvalitet             Finner kvalitetsopplysninger pÂ angitt nivÂ
CD ==========================================================================
CD FormÂl:
CD Finne kvalitetsopplysninger pÂ angitt nivÂ, hode ginfo eller pinfo.
CD
CD Parametre:
CD Type        Navn          I/U  Forklaring
CD --------------------------------------------------------------------------
...
CD                                 2 = ginfo
CD                                 3 = pinfo
CD                                 Returnerer aktuelt nivÂ.
CD long        pnr            i    punktnr. ved sp¯rring pÂ pinfo
CD short  *psMetode           u   Hvordan data er registrert.
CD                                 KVAL_MET_UNDEF  metode er udefinert.
CD long   *pLNnoyaktighet     u   Registreringsn¯yaktighet
CD                                 KVAL_NOY_UKJENT n¯yaktighet er ukjent.
                          ...
                          short sSynbarhet,short sHoydeMetode,long lHoydeNoyaktighet)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {            /* Aktuell gruppe OK */
      ...
      if ((neste = strstr(pp,"...KVALITET")) != NULL) {    /* Finn SOSI-navnet */
         ...
         *(cp+1) = '\0';
         ...
         if ((cp = strstr(neste+3,"...")) != NULL) {   /* Neste SOSI-navn */
            /* Heng pÂ resten av den gamle PINFOen */
            if (*temp)  UT_StrCat(temp," ",LC_MAX_SOSI_LINJE_LEN);
            UT_StrCat(temp,cp,LC_MAX_SOSI_LINJE_LEN);
         }

      /* KVALITET er ikke funnet, ta vare pÂ hele strengen */
      } else {
         UT_StrCopy(temp,pp,LC_MAX_SOSI_LINJE_LEN);
      }
      ...
      /* Hvis Kvaliteten avviker fra ginfo/hode, heng den pÂ PINFO */                                          /* Legg inn endringen */
...
CD strengen mÂ den kopieres over til egen streng. (Bruk UT_StrCopy).
CD
CD Leddnummer, delstreng,skilletegn og formateringskode kan inngÂ som
...
CD
CD Formateringskode. Dette brukes for Â angi plassering av komma og antall
...
CD                            og formateringskode kan inngÂ
CD                            som forlengelse av navnet.
CD                            OBS! Store og smÂ bokstaver er signifikante.
CD
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {                /* Aktuell gruppe OK */
      ...
      while (gi_lin <= siste_linje) {
         ...
         if ((strncmp(ginfo,szSosiNavn,sSosiLen) == 0) &&
            ...
            if (sLedd == 0) {
               ...
               //GÂr framover fra enden og fjerner anf¯rselstegn
            ...
            // Handter leddnummer
               ...
               while(s > 0) {
                  ...
                  if (cp != NULL) {
                     ...
                     if (*cp == '"') {
                        ...
                        rp = cp+1;              // UnngÂr start-hermetegnet
               ...
               if (cp != NULL) {
                  ...
                  *forste_linje = gi_lin;
                  ...
                  /* Husk linjenummeret */
                  *forste_linje = gi_lin;
                  break;           /* Funnet, ==> hopp ut av lõkken */
               }
            }
         }
      ...
      if ((rp != NULL) && (*rp != '\0')) { //22.11.2007 AR/ÅE Stopper tom variabel som har adresse
         ...
         if (sFormater == 2) { // Desimaltallet skal ikke avrundes
            // Fjerner un¯dvendige desimaler fra tall for Â unngÂ avrunding
            cp = rp;
            short sTeller=-99;
            while (*cp) {               
...
CD strengen mÂ den kopieres over til egen streng. (Bruk UT_StrCopy).
CD
CD Leddnummer, delstreng og formateringskode kan inngÂ som
CD forlengelse av SOSI-navnet.
CD
CD Leddnummer for flerleddet parameter angis ved #n.
...
CD                            inngÂ som forlengelse av navnet.
CD                            OBS! Store og smÂ bokstaver er signifikante.
      ...
      if (*pp == '"') {                 /* Hermetegn */
         if ((cp = strchr(pp+1,'"')) != NULL) {      /* Slutt-hermetegn */
            ...
            *cp++ = '\0';
            ...
            UT_ClrTrailsp(pp);         /* Fjern blanke pÂ slutten */
         }
...
CD Denne rutinen kan brukes til Â legge inn ginfo med nytt SOSI-navn.
...
CD Rutinen handterer at det er flere SOSI-navn pÂ samme linje.
CD OBS! Denne rutinen kan ikke brukes til Â legge inn nytt SOSI-navn.
CD
CD Parametre:
CD Type     Navn        I/U   Forklaring
CD --------------------------------------------------------------------------
...
CD ==========================================================================
CD FormÂl:
CD Legger inn nytt serienummer pÂ aktuell gruppe.
CD
CD Parametre:
CD Type     Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
/*
AR-930609
CH LX_PutSn                                           LavnivÂ Put Serienummer
CD ==========================================================================
CD FormÂl:
CD Legger inn nytt serienummer pÂ aktuell gruppe, uten Â sette flagg for at
...
CD Dette omfatter ogsÂ nytt gruppenavn (GINFO-linje 1)
CD Ønsker du Â endre serienummer mÂ LC_PutSn benyttes.
CD Referansenummer legges inn med LC_PutRef.
CD Blanke pÂ starten og slutten blir skrella vekk, og SOSI-navnet blir
...
SK_EntPnt_FYBA void LC_PutGi(short lin_nr, const char *pszGinfo)
{
   ...
   {   
      ...
      {                          
         // Gruppestart hÂndteres spesielt
			if (lin_nr == 1)
         {                   
...
/*
AR-930609
CH LX_PutGi                                          LavnivÂ put GINFO-linje
CD ==========================================================================
CD FormÂl:
CD LavnivÂ innlegging GINFO-linje rent generellt.
CD Blanke pÂ starten og slutten blir skrella vekk, og SOSI-navnet blir
CD konvertert til "store" bokstaver, ellers lagres det slik det er.
CD Sjekker ogsÂ at det aktuelle SOSI-navnet er lovlig Â oppdatere.
CD
CD Parametre:
CD Type     Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
static void LX_PutGi(short lin_nr, const char *szGinfo)
{
   ...
   /* Fjern blanke i starten og pÂ slutten */
   ...
   *ct = tmp;
   ...
   /* Annen GINFO */
      ...
      if (gr_start > 0) {
         /* ...ENHET i filhode pÂ fil med data */
         ...
         /* ...ENHET-H i filhode pÂ fil med data */
         ...
         /* ...ENHET-D i filhode pÂ fil med data */
         ...
         /* ..NGIS-LAG i filhode pÂ fil med data */
         ...
         /* ..ORIGO-NØ i filhode pÂ fil med data */
   ...
   if (sLagre == UT_TRUE) {
      ...
      /* Samme lengde, trenger ikke Â flytte noe i buffer */
      if (sNyLen == sGmlLen) {
      ...
      /* Siste linje i GINFO, trenger ikke Â flytte noe i buffer */
      ...
      /* MÂ flytte resten av buffer for Â tilpasse plassen */
      } else {
...
CD Dette omfatter ogsÂ serienummer og referansenummer.
...
SK_EntPnt_FYBA void LC_PutTH(long punkt_nr, double hoyde)
{
   if (Sys.GrId.lNr != INGEN_GRUPPE) {                  /* Aktuell gruppe OK */
      ...
      if (punkt_nr > 0 && punkt_nr <= Sys.pGrInfo->nko) {   /* Lovlig punkt ? */
         ...
         /* Gruppen har ..NAD fra f¯r og det pr¯ves Â legge inne en h¯yde */
...
SK_EntPnt_FYBA void LC_PutTD(long punkt_nr, double dybde)
{
   if (Sys.GrId.lNr != INGEN_GRUPPE){                  /* Aktuell gruppe OK */
      ...
      if (punkt_nr > 0 && punkt_nr <= Sys.pGrInfo->nko){    /* Lovlig punkt ? */
         ...
         /* Gruppen har ..NAH fra f¯r og det pr¯ves Â legge inn en dybde */
...
CD ==========================================================================
CD FormÂl:
CD Legger inn hele punktinformasjonen pÂ angitte punkt
      ...
      if (punkt_nr > 0 && punkt_nr <= Sys.pGrInfo->nko) {    /* Lovlig punkt ? */
         ...
         /* --------------------- Legg inn PINFO */
            ...
            if (((long)Sys.pGrInfo->ulPiLen - (long)sGmlLen + (long)sNyLen) < LC_MAX_PINFO_BUFFER) {
               ...
               if (sGmlLen == 0) {
                  /* S¯k mot starten av gruppen for Â finne slutten av forrige PINFO */
                  ulOfset = 0;
...
CD --------------------------------------------------------------------------
CD long    punkt_nr   i    Punktnummer (1 er f¯rste punkt)
CD short   sTestHoyde i    Bryter for Â si om h¯yde skal regnes med i sjekken.
...
/*
AR-911106
CH LC_BerLengde3D                       Beregn skrÂ lengde av aktuell gruppe
CD ==========================================================================
CD FormÂl:
CD Beregn skrÂ lengde av aktuell gruppe.
...
CD BÂde indre og ytre avgrensning beregnes.
CD
CD Parametre:
CD Type     Navn     I/U   Forklaring
CD --------------------------------------------------------------------------
