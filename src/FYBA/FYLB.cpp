...
extern volatile short fyba_initiert;    /* Bryter for Â vise at LC_Init er utf¯rt */
...
CD                               (Bruk "|" for Â kombinere.)
CD short          sStatus  r   Status  UT_TRUE=OK, UT_FALSE=ingen flere funnet
CD
CD Bruk:
CD     LC_FILADM *pFil;
CD               Denne l¯kka gÂr gjennom alle framgrunns-filene i basen
CD     LC_InitNextFil(&pFil)
...
CD Sekvensielle filer blir ikke hÂndtert.
CD
CD Parametre:
CD Type           Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
CD Bruk:
CD     LC_BGR Bgr;
CD               Denne l¯kka gÂr gjennom alle framgrunns-gruppene i basen
CD     LC_InitNextBgr(&Bgr);
...
CD                        GI_NAH   = gruppen har h¯yde informasjon (..NØH)
...
CD                           GI_PINFO = gruppen har PINFO
CD                           GI_NAH   = gruppen har h¯yde informasjon (..NØH)
CD                           GI_NAD   = gruppen har d¯bde informasjon (..NØD)
CD                           GI_KP    = gruppen har knutepunkt (...KP n)
CD                           GI_REF   = gruppen har referanser (.. :n)
CD                           GI_OY_REF= gruppen har referanser med ¯y
CD                           GI_NGIS      = gruppen er tatt ut fra NGIS for oppdat.
...
CD (Dette skjer f¯rst nÂr gruppen skrives til basen.)
CD
CD Parametre:
CD Type             Navn   I/U  Forklaring
CD --------------------------------------------------------------------------
...
CD                                Ut:  0=OK, -1=slutten av filen er nÂdd
...
CD                          GI_SLETTA    = gruppen er sletta (merka som sletta)
CD                          GI_READ_ONLY = gruppen kan ikke endres.
CD long             gml_snr u    Serienummer gruppen hadde pÂ ekstern fil
CD short            gnavn   r    Gruppenavn. (Se under $LENKE<LC_RxGr>)
CD
CD Bruk:
...
CD ikke aktuell filposisjon pÂ den sekvensielle filen.
CD Gruppen beholder serienummer hodet hadde pÂ SOSI-filen.
CD
CD Parametre:
CD Type             Navn   I/U  Forklaring
CD --------------------------------------------------------------------------
...
/*
AR:2000-10-17
CH LC_EndreHode                            Endre hodet pÂ eksisterende SOSI-fil
CD =============================================================================
CD FormÂl:
...
CD aktuell filposisjon pÂ den sekvensielle filen.
CD OBS!
CD Det mÂ vÊre nok ledig plass f¯r neste gruppe for tilbakeskrivingen.
CD Det er ikke mulig Â forandre koordinatsystem, enhet eller origo pÂ fil
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {                /* Aktuell gruppe OK */
      ...
      if (pFil->usLag != LC_SEKV) {
         ...
         if (Sys.pGrInfo->gnavn != L_HODE) {
            ...
            } else {        /* Det er ikke plass til Â skrive hodet */
...
CD ikke er i RB fra f¯r. (Styres ogsÂ av les_sosi.)
      ...
      if (Sys.sGrEndra != END_UENDRA) {
         if (!(!sNyGruppe  &&  les_sosi == LES_SOSI)) {
            if (LC_WxGr(SKRIV_OPTIMALT) == UT_FALSE) {
               // "Kan ikke lese ny gruppe. Du har ikke skriveaksess for Â lagre aktuell gruppe som er endret. Fil :"
...
SK_EntPnt_FYBA short LC_WxGr(short k_stat)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE)      /* Aktuell gruppe OK */
   {
      if (Sys.sGrEndra != END_UENDRA)     /* Gruppen er endra, mÂ skrive */
      {   
         /* Har skriveaksess? */
         ...
         {  
            ...
            if (Sys.sGrEndra == (short)END_ENDRA) {
               // Kontroller prikknivÂ i egenskaper ?
            ...
            // Filhode som ligger som f¯rste gruppe pÂ filen
...
static void LB_WriteRb(void)
{
   ...
   /* Er ikke pÂ filen fra f¯r, tildel ny plass */
   ...
   /* Er pÂ filen, sjekk om det er nok ledig plass */
   } else {
      ...
      /* Inne i buffer-filen */
         ...
         /* Det er ikke plass, mÂ flytte gruppen til slutten av filen */
...
SK_EntPnt_FYBA void LC_OppdaterEndret(short endring)
{
   ...
   if (gp == NULL){   
      ...
      if (szFlagg[0] == 'R'  ||  szFlagg[0] == 'H') {
         ...
         Sys.pGrInfo->info &= (unsigned short)(~GI_READ_ONLY); /* BÂde lese- og skriveaksess */
      }
   }
}
   ...
   /* Kopierer seg selv */
      ...
      *Sys.pGrInfo = *grtp;
      ...
      if (pBgr->lNr != 0)
      {
         ...
         {
            // Det skal aldri legges ut kvalitet pÂ .FLATE eller .OBJEKT
                  ...
                  long *nko,unsigned short *info)
{
   ...
   if (Sys.GrId.lNr == INGEN_GRUPPE){        /* Feil ==> Ingen aktuell gruppe */
      ...
      if (Sys.pGrInfo->nko > l) {   /* Har det blitt lagt inn flere linjer */
         ...
         if (Sys.GrId.pFil->sSosiVer < 400)
         {
            // Husk kvalitet i GINFO pÂ aktuell gruppe
            nivaa = 2;
            pnr = 1;
...
CD BÂde koordinater, h¯yde, KP og PINFO blir behandlet.
CD For .BUE blir fortegnet pÂ radius endret.
CD Fortegnet pÂ referanser til gruppen blir oppdatert.
CD
CD Parametre:
CD Type    Navn     I/U   Forklaring
CD -----------------------------------------------------------------------------
...
SK_EntPnt_FYBA void LC_SnuGr(void)
{
   ...
   /* Lovlig gruppe */
      ...
      /* Legger inn koordinater og PINFO */
		...
		/* ====== Hvis gruppen er .BUE ==> skift fortegn pÂ radius ==== */
...
CD disse kan leses.)
CD Det er ikke mulig Â slette grupper fra sekvensielle filer, eller grupper
                  ...
                  if (sStatus == UT_TRUE) {
                     ...
                     if (Sys.pGrInfo->sosi_st == NY_SOSI_ST  ||
                        ...
                        if (Sys.pGrInfo->sosi_st != NY_SOSI_ST) { /* Ikke ny gruppe */
                           ...
                           /* Posisjoner og blank ut omrÂdet */
...
CD Siste del av gruppen legges som en ny gruppe pÂ samme fil som
CD opprinnelig gruppe. Denne blir skrevet til basen.
CD Den delen av gruppen som ligger mellom P1 og P2 blir fjernet.
CD
CD Hvis gruppen er BUEP og en av delene fÂr bare to koordinater
CD blir det lagt inn et nytt punkt midt pÂ buen.
CD
CD Parametre:
CD Type     Navn   I/U   Forklaring
CD --------------------------------------------------------------------------
CD long    sP1     i    Punktnummer 1.   (MÂ vÊre st¯rre enn 1)
CD long    sP2     i    Punktnummer 2.   (MÂ vÊre mindre enn nko)
...
SK_EntPnt_FYBA short LC_SplittGr (long sP1,long sP2,LC_BGR * pBgr2)
{
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE  &&
      ...
      if (Sys.pGrInfo->gnavn == L_BUE  || 
          ...
          Sys.pGrInfo->gnavn == L_SIRKELP )
      {
         // Tar vare pÂ bueparametrene for opprinnelig "bue"
      ...
      if (Sys.pGrInfo->gnavn == L_BUE  &&  bStorbue)
      {
         ...
         /* Buen skal ikke vÊre storbue lenger hvis sentrum er flyttet.
          * Dette er tilfelle nÂr linjen fra gammelt til nytt senter
          * skjÊrer linjen fra nytt start til sluttpunkt.
          */
      ...
      if (Sys.pGrInfo->gnavn == L_BUEP)
      {
         if (nko == 2)
         {
            // --- Legger inn nytt punkt pÂ buen mellom de to endepunktene.
      ...
      // SIRKELP mÂ konverteres til BUEP
      if (Sys.pGrInfo->gnavn == L_SIRKELP )
      {
         LC_PutGi(1,".BUEP");
      }
...
static void LR_TestEndreBuepTilKurve(double dDeltaFi)
{
   ...
   {
      // Buen er snudd krumming (Âpningsvinkelen har skiftet fortegn)
      if (dDeltaFi * dfi  <  0.0)
      {
         LC_PutGi(1,".KURVE");
      }
      else
      {
         ...
         // Avstand fra fotpunktet til punktet pÂ buen mÂ vÊre minst en enhet
                    ...
                    short *ngi,long *nko,unsigned short *info)
{
   ...
   if (Sys.GrId.lNr == INGEN_GRUPPE){        /* Feil ==> Ingen aktuell gruppe */
      ...
      if (LC_CopyCoord(pFraBgr,retning,sTilPkt,ngi,nko,info)) {
         ...
         if (LC_FFFlate(&GeoStat,&FlateBgr)) {
            do {
               if (FlateBgr.pFil == AktBgr.pFil) {  /* PÂ samme fil ? */
...
static short LB_RensOmkrets(LC_POL_OMKR * pPO,long lAktSnr,long lFraSnr)
{
   ...
   if (pPO != NULL) {
      /* Tilslag pÂ f¯rste og siste gruppe */
...
CD Sjekker ledig plass bÂde for indeks-fil og sosi-fil.
CD
CD Parametre:
CD Type       Navn    I/U   Forklaring
CD -----------------------------------------------------------------------------
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {            /* Aktuell gruppe OK */
      if (linje > 0  &&  linje <= (Sys.pGrInfo->ngi + 1)) {
         ...
         /* Inni GINFO */
            ...
            Sys.sGrEndra = (short)END_ENDRA;
            /* MÂ flytte resten av buffer for Â gi plass til de nye linjene */
...
/*
AR-930610
CH LC_AppGiL                                             Heng pÂ en GINFO-linje
CD =============================================================================
CD FormÂl:
CD Henger pÂ en linje i GINFO-delen i en gruppe.
CD
CD Parametre:
CD Type     Navn    I/U   Forklaring
CD -----------------------------------------------------------------------------
CD short    ngi      r    Antall GINFO-linjer i gruppen etter utvidelsen.
CD                        (Linjenumret pÂ den tilf¯yde linjen.)
CD
CD Bruk:
CD ngi = LC_AppGiL();
   =============================================================================
...
/*
AR-911001
CH LC_AppKoL                                       Heng pÂ en koordinatlinje
CD ==========================================================================
CD FormÂl:
CD Henger pÂ en linje i koordinatdelen i en gruppe.
CD
CD Parametre:
CD Type     Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
   ...
   if (Sys.GrId.lNr != INGEN_GRUPPE) {              /* AKtuell gruppe OK */
      ...
      if (antall > 0) {
         ...
         if (Sys.pGrInfo->ulPiLen > 0) {
            /* Er det PINFO pÂ de punktene som er slettet? */
            pI = pInfo;
            ulP1 = LC_INGEN_PINFO;
...
CD Eventuelle gamle pekere mÂ vÊre frgitt utenfor denne rutinen.
CD
CD Parametre:
CD Type       Navn    I/U  Forklaring
CD --------------------------------------------------------------------------
CD LC_FILADM *pFil     i   Peker til FilAdm
CD UT_INT64   start    i   Startposisjon pÂ SOSI-filen
CD UT_INT64   slutt    u   Sluttposisjon pÂ SOSI-filen
CD short      siste_gr r   Siste gruppe (0=ikke siste, 1=siste pÂ filen)
CD
CD Bruk:
         ...
         if (type != LEST_BLANK) {
            ...
            if (type == LEST_GINFO) {
               ...
               if (pLb->cur_navn[pLb->cur_niv-1] == L_REF1  ||
                  ...
                  /* ..REF skal skrives bare pÂ f¯rste linje med referanser */ 
...
void LB_Swap(void)
{
   ...
   if ( ! siste ) {
      ...
      if ( ! LB_WGru(SKRIV_VANLIG,1,nko,Sys.GrId.pFil,start,&neste)) {
         /* For lite plass, blank ut "gammelt" omrÂdet pÂ SOSI-filen */
         _fseeki64(Sys.GrId.pFil->pBase->pfSos,start,SEEK_SET);
         Sys.GrId.pFil->pBase->BufAdm.sStatus = LESEBUFFER_TOM;
         LB_WriteBlank(Sys.GrId.pFil->pBase->pfSos,Sys.GrId.pFil->sTegnsett,neste); /* Blank ut omrÂdet */

         /* Gruppen skrives pÂ slutten av SOSI-filen */
         siste = 1;
         start = Sys.GrId.pFil->n64AktPos;
      }
   }

   /* Siste gruppe pÂ SOSI-filen */
   if ( siste ) {
...
CD UT_INT64   ffipos    i   Startposisjon pÂ SOSI-filen
CD UT_INT64  *lfipos   iu   inn: F¯rste pos i neste gruppe (mÂ ikke overskrives)
CD                          ut : F¯rste pos etter gruppen (etter event. utrop.)
CD short      ist       r   Status: 1 = Skrevet OK
CD                               0 = Ikke plass, mÂ flyttes til slutten
CD
CD Bruk:
...
/* printf("\nWGru:fra: %s : %ld nko:%ld til %s",
   ...
   for (i = 1; i <= ngi; i++) {
      ...
      */
      ...
      while (nfipos > *lfipos) {
         if (gnavn == L_HODE) {
            /* Pr¯ver Â flytte gruppen til slutten */
   ...
   forrige_nah = -1;
   skriv_nah = 1;         /* Skriv ..NØ(H) f¯r f¯rste koordinat linje */

   /* =====> Skriv koordinater og PINFO */
   for (l=0; l<antall; l++,pt++) {
      nah = ((Sys.pInfo + pt)->dHoyde != HOYDE_MANGLER);

       /* MÂ skrive ..NØ eller ..NØH ved skifte mellom disse */
      if (forrige_nah != nah) {
         skriv_nah = 1;
      }
   ...
   if (strategi == SKRIV_SISTE)                 /* PÂ slutten av filen */
   {
      /* Akt.pos + reserveplass */
      *lfipos = _ftelli64(pfSos) + (UT_INT64)Sys.sResPlass;
      /* Legg pÂ 200 ekstra etter filhodet */
      ...
      if (sFilMindre == UT_TRUE  ||  Sys.sUtvidModus == LC_UTVID_SIKKER) {
         ...
         /* Steng filen for Â bevare byte-pekeren */
...
CD --------------------------------------------------------------------------
CD LC_FILADM *pFil     i    Peker til FilAdm
CD UT_INT64   start    i    Startposisjon pÂ SOSI-filen (f¯rste pos. i gr.)
...
static short LB_FlyttGrTilSlutt(LC_FILADM *pFil, UT_INT64 start, UT_INT64 *neste)
{
   ...
   /* Sjekk gruppetabellen for Â finne hvilken gruppe */
...
CD ==========================================================================
CD FormÂl:
CD Fyller omrÂdet FRA-OG-MED current-posisjon og fram TIL,
...
/*
AR-911001
CH LB_WriteLine                                    LavnivÂ skriv tekst linje
CD ==========================================================================
CD FormÂl:
CD LavnivÂ overbygning ove write i C-biblioteket for Â skrive en linje.
CD
CD Parametre:
CD Type     Navn     I/U   Forklaring
CD --------------------------------------------------------------------------
...
SK_EntPnt_FYBA void LC_ErstattReferanse (LC_FILADM *pFil,long lGmlSnr,long lNyttSnr, bool bSammeRetning)
{
   ...
   while (LC_NextBgr(&Bgr,LC_FRAMGR))
   {
      if (Bgr.pFil == pFil)
      {
         ...
         if ((info&GI_REF) != 0)
         {
            // Gruppen er pÂ rett fil og har referanser, Bytt serienummer 
            LC_RxGr(&Bgr,LES_OPTIMALT,&ngi,&nko,&info);
            lAntRef = LC_InqAntRef();
            plRefArray = (long *) UT_MALLOC(lAntRef * sizeof(long));
            sGiLin = 2;
