...
/*
SJM-931003
CH LC_POL_FjernOy                    Fjernar ei oy frÂ kjede av ¯yelement
CD =======================================================================
CD Bruk:
CD LC_OY_ADM  OyKjede;
...
CD =======================================================================
CD FormÂl:
CD Frigir minne som er allokert til polygon. (BÂde omkrets og hull.)
CD
CD Parametere:
CD Type        Navn     I/U    Forklaring
CD -----------------------------------------------------------------------
...
{
   #define MAX_LEN  66   /* Ginfolinjen skrives ut nÂr den er lengre en 70 tegn */
   LC_POL_ELEMENT *pPE;
   LC_OY_ELEMENT *pOE;
   short gilin;
...
CD                               UT_TRUE = punktet ligger inne pÂ flaten
CD
CD Bruk:
CD .
==========================================================================
...
SK_EntPnt_FYBA short LC_POL_PTst(LC_POLYGON *pPolygon,double a,double n)
{
   ...
   short inni = UT_FALSE;   /* Returverdi: 1 = inne pÂ flaten, 0 = utenfor  */
...
SK_EntPnt_FYBA short LC_POL_PTstOmkrets(LC_POL_OMKR *pPO,double a,double n)
{
   ...
   /*
    * Sjekk omkretsen ved Â beregne antall skjÊringer mellom omkretsen
    * og en linje fra "test-punktet" til "uendelig ¯st".
    */

   for (pPE = pPO->pForstePE; pPE != NULL; pPE = pPE->pNestePE) {
      /* Beregn skjÊringer med denne gruppen */
      sAntSkjaer += LR_PTstGruppe(&pPE->Bgr,a,n);
   }
    ...
    * (Inni hvis antall skjÊringspunkt er oddetall.)
    */
...
/*
JAØ-20061130
CH LC_POL_OmkretsSkjaering        Finner antall skjÊringer med polygonomkrets
CD ==========================================================================
CD FormÂl:
CD Sjekker om gitt punkt ligger innenfor yttergrensen for polygon angitt
CD av struktur. Egentlig kopi av LC_POL_PTstOmkrets, men returnerer antall 
CD skjÊringer istedet for inni/utenfor. 
CD Forutsetter at tabellen danner et lukket polygon
CD Skifter ikke aktuell gruppe.
CD
CD Parametre:
...
CD double        a      i    Punkt som skal sjekkes
CD double        n      i
CD short         ist    r    Antall skjÊringer med omkrets fra pkt til "uendelig" ¯st
CD
CD Bruk:
CD ist = LC_POL_PTstOmkrets(pPO,a,n);
   ==========================================================================
