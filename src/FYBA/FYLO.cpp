...
volatile short fyba_initiert = 0;    /* Bryter for Â vise at LC_Init er utf¯rt */
...
SK_EntPnt_FYBA void LC_SetFilLag(LC_FILADM *pFil,unsigned short usLag)
{
   ...
   /* MÂ lagre aktuell gruppe hvis den er pÂ denne filen og er endret */
   ...
   /* Steng eventuell Âpen fil */
   LO_CloseSos(pFil->pBase);

   if ( usLag == LC_FRAMGR) {
      /* Sjekk at filen kan Âpnes med den ¯nskede aksessen */   
...
CD Kopien legges pÂ en underkatlog med navn "Backup" under den katalogen
CD SOSI-filen ligger pÂ. Kopien navnes "Filnavn.nnn" der nnn er et
...
SK_EntPnt_FYBA short LC_Backup(LC_FILADM *pFil, const char *pszBackupPath)
{
   ...
   for (sIdx=0; sIdx<100; sIdx++) {
      ...
      /* (Gj¯res ved Â pr¯ve Â sp¯rre om filens st¯rrelse) */
...
CD ==========================================================================
CD FormÂl:
CD Velger handteringsmÂte for grupper som er merket for oppdatering av NGIS.
CD	Standardverdi fra LC_Init er NGIS_NORMAL.
CD
CD Parametre:
CD Type   Navn  I/U  Forklaring
CD --------------------------------------------------------------------------
CD short  modus  i   BehandlingsmÂte:
CD						    NGIS_NORMAL  (0) = Vanlig handtering
CD							 NGIS_SPESIAL (1) = Spesialmodus der det er mulig Â lese
...
CD ==========================================================================
CD FormÂl:
CD Henter handteringsmÂte for grupper som er merket for oppdatering av NGIS.
CD	Standardverdi fra LC_Init er NGIS_NORMAL.
CD
CD Parametre:
CD Type   Navn  I/U  Forklaring
CD --------------------------------------------------------------------------
CD short  modus  r   BehandlingsmÂte:
CD						    NGIS_NORMAL  (0) = Vanlig handtering
CD							 NGIS_SPESIAL (1) = Spesialmodus der det er mulig Â lese
CD                                       grupper som er merka som sletta.
CD
...
CD Dette blir ¯delagt ved neste kall til en "get-rutine". For Â ta vare pÂ
CD strengen mÂ den kopieres over til egen streng. (Bruk strcpy).
CD
CD Parametre:
CD Type       Navn    I/U  Forklaring
CD --------------------------------------------------------------------------
...
CD ==========================================================================
CD FormÂl:
CD Velger handteringsmÂte for utvidelse av SOSI-filer.
...
CD                                          skrevet pÂ slutten av filen.
CD							 LC_UTVID_RASK   (1) = SOSI-filen stenges IKKE etter hver
CD                                          gruppe som er skrevet pÂ slutten
CD                                          av filen.
CD                                          (MÂ bare brukes i spesielle tilfeller.)
CD
CD Bruk:
...
CD ==========================================================================
CD FormÂl:
CD Henter valgt handteringsmÂte for utvidelse av SOSI-filer.
CD
CD Parametre:
CD Type   Navn  I/U  Forklaring
CD --------------------------------------------------------------------------
...
CD OBS! All aktivitet mot basen mÂ vÊre avsluttet f¯r dette kallet.
CD (Alle sosi-filer mÂ vÊre stengt!)
CD
CD Parametre:
CD Type        Navn     I/U  Forklaring
CD --------------------------------------------------------------------------
CD LC_BASEADM * pBase     i   Peker til baseadministrasjonsblokk.
CD short       sStatus   r   UT_TRUE=OK, UT_FALSE=Basen har Âpne filer.
CD
CD Bruk:
...
CD Kladdefilen legges pÂ current directory.
CD Kladdebase brukes bare nÂr alle SOSI-filer Âpnes med sekvensiell les/skriv.
CD  
...
CD                                        tilfeller hvis ingen fil Âpnes
CD                                        som LC_BASE_xx.
CD LC_BASEADM * pBase      r   Basepeker.
CD
CD Bruk:
...
CD Kladdefilen legges pÂ samme directory som indeksfilene.
CD
CD Parametre:
CD Type        Navn      I/U   Forklaring
CD --------------------------------------------------------------------------
CD LC_BASEADM * pBase      i    Basepeker.
CD short       status     r    Status. UT_TRUE=OK, UT_FALSE=Feil ved Âpning
CD
CD Bruk:
...
static short LO_OpenKladd(LC_BASEADM * pBase)
{
   ...
   if (o_stat == UT_OK){
      ...
      if (LC_OpenSos(fil,LC_BASE_FRAMGR,LC_NY_IDX,LC_INGEN_STATUS,
         ...
         /* Merke for at dette er Âpen kladdebase */
...
CD OBS! All aktivitet mot filen og indeksfilene mÂ vÊre avsluttet f¯r dette
CD kallet.
CD szBaseVer settes til '\0', slik at dette kan brukes for Â sjekke om en
                                ...
                                LC_FILADM **pFil, short *o_stat)
{
   ...
   *o_stat = 0;
   LO_CloseSos(Sys.pAktBase);            /* Steng eventuell Âpen fil */
   ...
   /*
    * Fil som bare skal leses mÂ finnes fra f¯r.
    */
   ...
   /*
    * Sekvensiell utvid krever at filen finnes og at det er lov Â skrive.
    */
   ...
   UT_StrCopy(pFi->szDato,"*",LC_DATO_LEN);
	...
	pFi->lAntGr = 0L;
	pFi->sTegnsett = TS_DOSN8; /* Standard nÂr ikke annet er gitt */
   ...
   *pFi->szNgisLag = '\0';
   ...
   if (*o_stat == UT_OK) {      /* Gammel file er Âpnet OK */
      ...
      /* Åpningsfeil pÂ SOSI-filen  ==>  avbryter */
      char szError[256];
      UT_strerror(szError,256,*o_stat);
   ...
   /* Eksisterende fil er Âpnet OK, hent tegnsett */
   if (*o_stat == UT_OK) {
      ho_GetTegnsett(pFi->pBase->pfSos,&(pFi->sTegnsett));

   /* Tom fil, pr. def. DOSN8 nÂr ikke gitt annet tegnsett */ 
   } else if (*o_stat == -1) {
   ...
   /* OmrÂde */
   if (*o_stat == UT_OK) {
      /* Eksisterende fil er Âpnet OK, hent omrÂde */
      ...
      if (sModus == LC_SEKV_UTVID) {
         ...
         ho_GetKvalitet(pFi->pBase->pfSos,&(pFi->Kvalitet.sMetode),
			...
			pFi->n64AktPos = 0L;
                     ...
                     /* nÂr hodet leses/skrives fra/til SOSI-filen */
         /* Denne brukes senere for Â sjekke at hode er skrevet til filen */
   ...
   if (sModus == LC_BASE_FRAMGR  ||  sModus == LC_BASE_BAKGR) {
      ...
      if (sNyIdx == LC_GML_IDX) {
         ...
         if (sStatus == UT_OK) {
            ...
            // Filen er Âpen i et annet program
            LO_CloseSos(Sys.pAktBase);  // Steng eventuell Âpen fil
            ...
            *o_stat = LC_OPPTATT;
            ...
            // Feil indeksversjon, st¯rrelse eller oppdateringstid pÂ SOSI-filen
				// - MÂ bygge ny indeks
            LI_Close(pFi,RESET_IDX);
            sNyIdx = LC_NY_IDX;

            /*
            * MÂ initiere deler av fil-adm pÂ nytt.
            */
            pFi->lAntGr = 0L;
            pFi->n64NesteLedigRbPos = 0;
            pFi->lMaxSnr = NYTT_SNR;   /* Ikke noe akt. snr */
...
/*
AR-910925
CH LO_ReopenSos                                              ReÂpne SOSI-fil
CD ==========================================================================
CD FormÂl:
CD Åpner pÂ nytt en av filene i filtabellen.
CD
CD Parametre:
CD Type         Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
SK_EntPnt_FYBA void LC_CloseSos(LC_FILADM *pFil,short s_stat)
{
   ...
   /* LO_TestFilpeker(pFil,"LC_CloseSos"); */
	...
	/*
    * Hvis aktuell gruppe er pÂ denne filen
    */
...
/*
AR:2009-03-20
CH LC_FcloseSos                             Steng Âpen SOSI-fil i filsystemet
CD ==========================================================================
CD FormÂl:
CD Denne rutinen brukes til Â sikre at en fil er oppdatert og stengt i filsystemet,
CD slik at SOSI-filen kan leses fra andre program.
CD 
CD OBS!
CD Stenger bare filen i filsystemet.
CD Filen er fortsatt Âpen i basen.
CD FYBA Âpner automatisk filen pÂ nytt nÂr det er behov for dette.
CD
CD Parametre:
CD Type        Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
CD som mÂ vÊre SOSI-filens hode.
CD
CD Parametre:
CD Type         Navn    I/U   Forklaring
CD --------------------------------------------------------------------------
...
/*
AR-911003
CH LC_GetBaOm                                                Hent baseomrÂde
CD ==========================================================================
CD FormÂl:
CD Henter baseomrÂdet for aktuell base (Sum av filhodene).
...
CD double        *oha      u    Øvre h¯yre ¯st
CD double        *ohn      u    Øvre h¯yre nord
...
/*
AR-910928
CH LC_GetFiOm                                       Hent omrÂde fra fil-hode
CD ==========================================================================
CD FormÂl:
CD Hent omrÂde for en SOSI-fil i basen.
...
CD Aktuell gruppe blir brukt under prosessen, og etterpÂ er det ingen
...
static short LO_InklSos(LC_FILADM *pFil,short sVisStatus)
{
   ...
   LO_ReopenSos(pFil);    // Sikkrer at filen er Âpen - Kan ha blitt stengt i LC_WxGr
...
CD ==========================================================================
CD FormÂl:
CD Henter filnavnet for en fil i basen. Fungerer bÂde for basefiler
CD og for sekvensielle filer.
CD OBS! Hvis du skal endre pÂ filnavnet mÂ du f¯rst kopiere det 
...
CD
CD Denne rutinen er primÊrt tenkt brukt i GabEdit hvor det er behov for Â 
...
CD disse alltid slettet nÂr SOSI-filen stenges.
CD OBS! Ingen SOSI-filer kan vÊre Âpne nÂr indekskatalog velges.
CD
CD Parametre:
CD Type        Navn       I/U Forklaring
CD --------------------------------------------------------------------------
CD const char *pszIdxPath  i  Katalog for indeks
CD                            "" = Tom streng betyr at indeksfilene legges pÂ en
CD                                 underkatalog under katalogen der SOSI-filen ligger
CD                                 (default).
CD                            "katalognavn" = Indeksfilene legges pÂ en
...
SK_EntPnt_FYBA short LC_SetIdxPath(const char *pszIdxPath)
{
   if (LC_InqAntFiler(LC_FRAMGR|LC_BAKGR) == 0)
   {
      ...
      /*
      CD "TEMP" = Indeksfilene legges pÂ brukerens temp-katalog.
