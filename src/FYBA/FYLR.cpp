...
CD ==========================================================================
CD FormÂl:
CD Beregner og lagrer omskrevet boks for koordinatene pÂ aktuell gruppe.
CD
CD Parametre: ingen
CD
CD Bruk:
...
void LR_Indx(void)
{
   ...
   if (Sys.pGrInfo->nko > 0) {
      ...
      Boks.dMinNord = min_n - Sys.pGrInfo->dEnhet;
         /* Øvre h¯yre justeres ut en enhet */
...
void LR_IndxFlate(void)
{
   ...
   if ((info & GI_REF) != 0  ||  nko >0) {
      ...
      /* Koordinater direkte pÂ denne gruppen */
      if (nko > 0) {
         ...
         if (pRL != NULL) {
            ...
            /* Initier med inverse omrÂde fra filhodet */
       ...
       * firkant for gruppene som inngÂr.
...
CD =============================================================================
CD FormÂl:
CD Definerer geografisk omrÂde for geografisk s¯k.
CD
CD Parametre:
CD Type           Navn      I/U  Forklaring
CD -----------------------------------------------------------------------------
              ...
              double nv_a,double nv_n,double oh_a,double oh_n)
{
   ...
   /* Avrunder ut til nÊrmeste meter utover */
   /* Nedre venstre ut til nÊrneste heltall under */
   //pGeoStat->nvn = (long)floor(nv_n);
   //pGeoStat->nva = (long)floor(nv_a);
   /* Øvre h¯yre ut til nÊrneste heltall over */
...
CD ==========================================================================
CD FormÂl:
CD Finner f¯rste gruppe i det definerte omrÂdet for kombinert geografisk s¯k.
CD
CD Parametre:
CD Type     Navn      I/U  Forklaring
CD --------------------------------------------------------------------------
   ...
   if (pGeoStat->usMetode == LC_GEO_RTRE) {
      ...
      LC_InitNextFil(&pFil);
	   while (LC_NextFil(&pFil,pGeoStat->usLag)) {
         /* Filen inneholder data, mÂ sjekke alle ber¯rte noder */
         if (pFil->pGeoRN != NULL) {
...
CD som overlapper s¯kerektanglet. De gruppene som blir funnet blir hengt pÂ
...
CD ==========================================================================
CD FormÂl:
CD Finner neste gruppe i det definerte omrÂdet for geografisk s¯k.
CD
CD Parametre:
CD Type           Navn      I/U  Forklaring
CD --------------------------------------------------------------------------
   ...
   if (pGeoStat->usMetode == LC_GEO_RTRE) {
      ...
      /* Sjekker den aktuelle filen */
	   if (pOnsketFil->usLag & pGeoStat->usLag) {
         /* File inneholder data, mÂ sjekke alle ber¯rte noder */
...
CD ==========================================================================
CD FormÂl:
CD Finn alle i geografisk s¯keomrÂde.
...
/*
AR-911003
CH LC_SBFlate                     Sett s¯kegrense for geografisk s¯k pÂ flate
CD =============================================================================
CD FormÂl:
CD Definerer punkt for geografisk s¯k pÂ flate.
CD
CD Parametre:
CD Type     Navn      I/U  Forklaring
CD -----------------------------------------------------------------------------
...
CD =============================================================================
CD FormÂl:
CD Finner f¯rste gruppe i det definerte omrÂdet for flates¯k.
CD
CD Parametre:
CD Type     Navn      I/U  Forklaring
CD -----------------------------------------------------------------------------
   ...
   if (pGeoStat->usMetode == LC_GEO_RTRE) {
      ...
      LC_InitNextFil(&pFil);
	   while (LC_NextFil(&pFil,pGeoStat->usLag)) {
         ...
         /* File inneholder data, mÂ sjekke alle ber¯rte kvadranter */
         if (pFil->pGeoRN != NULL) {
...
CD ==========================================================================
CD FormÂl:
CD Finner neste gruppe i det definerte omrÂdet for flates¯k.
CD
CD Parametre:
CD Type     Navn      I/U  Forklaring
CD --------------------------------------------------------------------------
...
CD                               1 = punktet ligger inne pÂ flaten
CD
CD Bruk:
CD .
...
{
   LC_POLYGON Polygon;
   short inni = 0;   /* Returverdi: 1 = inne pÂ flaten, 0 = utenfor  */
...
SK_EntPnt_FYBA short LC_PTstOmkrets(double a,double n)
{
   ...
   // Pluss pÂ et lite tillegg for Â unngÂ treff pÂ node
   a += dEnhet / 1000.0;
   n += dEnhet / 1000.0;
...
CD ==========================================================================
CD FormÂl:
CD Sjekker antall skjÊringer mellom gitt gruppe og linje fra gitt punkt
CD til "uendelig" ¯st.
CD
CD Parametre:
CD Type    Navn       I/U   Forklaring
...
CD double  a           i    Punkt som skal sjekkes
CD double  n           i
CD short   sAntSkjaer  r    Antall skjÊringer
CD
CD Bruk:
...
LC_R_LEAF * LR_InsertGeo(LC_FILADM *pFil,long lNr,LC_BOKS * pB)
{
   ...
   /* Hvis rot-noden er splittet mÂ det lages ny rot-node */
   if (pNyRN != NULL) {
      if (pFil->pGeoRN != NULL) {
         /* Lag ny rot-node */
...
CD Hvis pRN == NULL (Tomt tre) settes *ppNyRN til Â peke til et nyt tre.
CD =======================================================================
*/
static LC_R_LEAF * LR_R_Insert(long lGrNr,LC_BOKS *pB,LC_R_NODE *pFar,LC_R_NODE *pRN,LC_R_NODE **ppNyRN)
{
   ...
   //static int iNivo;
   //iNivo++;
   //UT_FPRINTF(stderr,"LR_R_Insert nivÂ :%d\n",iNivo);

   
   *ppNyRN = NULL;


   /* Treet er tomt, mÂ allokere ny node */
   if (pRN == NULL) {
   ...
   if (pRN->sSonType == LC_LEAF) {
      ...
      /* Fullt, mÂ opprett ny node */
      } else {
         *ppNyRN = pNyRN = LR_R_CreateRNode(pRN->pFar,LC_LEAF);

         /* Fordel boksene pÂ de to nodene etter lineÊr kost algoritmen */
   ...
   /* Node */
      ...
      /* Denne noden har blitt splittet, dette mÂ tas vare pÂ her */
      if (*ppNyRN != NULL) {
         ...
         if (pRN->sSonAnt < LC_R_MAX_SON) {
            ...
            /* Ingen splitting pÂ nivÂet over */
            *ppNyRN = NULL;
         ...
         /* Fullt, mÂ opprett ny node */
            ...
            /* Husk den nye noden til nivÂet over */
            *ppNyRN = pNyRN;
         }
...
CD =======================================================================
CD FormÂl:
CD Summerer to bokser ved at boks1 blir utvidet slik at den ogsÂ dekker 
CD boks2.
CD
CD Parametere:
CD Type       Navn  I/U  Forklaring
...
void LR_R_Delete(LC_R_LEAF * pRL)
{
   ...
   if (pRL != NULL) {
      ...
      while ( iFerdig == UT_FALSE) {
         ...
         /* Antall s¯nner er 0, denne noden skal fjernes pÂ nivÂet over */
         if (pRN->sSonAnt == 0) {

            // Er ikke pÂ ¯verste nivÂ
            if (pRN->pFar != NULL)  {
            ...
            /* Har kommet til toppen */
               ...
               /* Treet er tomt initierer verdier pÂ nytt */
         ...
         /* Har flere s¯nner, mÂ beregne omskrevet boks pÂ nytt */
         } else {
...
CD s¯keomrÂdet er mindre enn 10% av baseomrÂdet i nord eller ¯st retning.
CD
CD Parametre:
CD Type     Navn      I/U  Forklaring
CD --------------------------------------------------------------------------
