...
CD Ved rydding av anf¯rseltegn ryddes ogsÂ tegn inni strengen.
CD Ved rydding mÂ parameterstrengen bestÂ av mer enn ett tegn.
CD
CD PARAMETERLISTE:
...
CD short  sMaksLen    i    Makslimal lengde pÂ streng inklusiv null-termiantor
CD short  sOk         r    Feilstatus. UT_OK hvis ok, UT_ERROR hvis strengen er
CD                         for kort til Â f¯ye pÂ anf¯rselstegn.
CD
CD Bruk:
...
SK_EntPnt_UT short UT_PutAnfTegn(char *pszTx, char cTegn, short sMaksLen)
{
   ...
   /* Hvis det skal legges pÂ " eller ' i para-strengens ender: */
   if (cTegn=='\"' || cTegn=='\'') {
      ...
      // Strengen bestÂr av bare et anf¯rselstegn.
      } else if ((sTxtLen == 1 ) && (pszTx[sTxtLen - 1]=='\'' || pszTx[sTxtLen - 1]=='\"')){
          if (sTxtLen <= sMaksLen - 2)
          {
   ...
   /* Fjerner appostroffer: */
       ...
       if (sTxtLen <= sMaksLen)
       {
          /* Para-strengen mÂ innholde mer enn ett tegn: */
          if (sTxtLen > 1) {
